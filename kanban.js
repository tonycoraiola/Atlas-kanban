...
function addCardToState(data, silent = false) {
  const card = {
    ...
    created: data.created || new Date().toLocaleDateString('pt-BR', { month: 'short', year: 'numeric' }),
  };
...
function renderKanban() {
  ['todo', 'doing', 'done'].forEach(col => {
    ...
    const colCards  = State.cards.filter(c => c.col === col && !c.deleted);
...
function renderCard(c) {
  ...
  const deleteBtn = `<button class="card-btn delete-btn" onclick="deleteCard(${c.id})" title="Mover para lixeira">🗑</button>`;
  ...
  return `<div class="card" draggable="true" data-id="${c.id}">
    ...
    <div class="card-footer">
      ...
      <div class="card-actions">${moveBtn}${doneBtn}${deleteBtn}</div>
    </div>
  </div>`;
}

// ═══════════════════════════════════════════════
// AÇÕES DOS CARDS — com sync API
// ═══════════════════════════════════════════════
...
async function markDone(id) {
  ...
  } catch (e) { console.error('Erro ao atualizar card:', e); }
...
async function deleteCard(id) {
  ...
  } catch (e) { console.error('Erro ao excluir card:', e); }

  card.deleted = true;
  showToast('🗑', `<strong>Card movido para lixeira</strong> ${card.title}`);
  renderKanban();
  renderSidebar();
  renderOverview();
}
...
async function restoreCard(id) {
  ...
  try {
    await fetch(`${API_URL}/cards/${id}/restore`, { method: 'PUT' });
  } catch (e) { console.error('Erro ao restaurar card:', e); }

  card.deleted = false;
  showToast('♻️', `<strong>Card restaurado!</strong> ${card.title}`);
  renderKanban();
  renderSidebar();
  renderOverview();
}
...
async function permanentDeleteCard(id) {
  ...
  if (!confirm(`Excluir permanentemente "${card.title}"? Esta ação não pode ser desfeita.`)) return;

  try {
    await fetch(`${API_URL}/cards/${id}/permanent`, { method: 'DELETE' });
  } catch (e) { console.error('Erro ao excluir permanentemente:', e); }

  State.cards = State.cards.filter(c => c.id !== id);
  State.notes = State.notes.filter(n => n.cardId !== id);
  showToast('🗑', `<strong>Card excluído permanentemente</strong>`);
  renderKanban();
  renderSidebar();
  renderOverview();
}
...
async function onDrop(e, col) {
  ...
  try {
    await fetch(`${API_URL}/cards/${dragId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ col }),
    });
...
async function addCard() {
  ...
  try {
    const res = await fetch(`${API_URL}/cards`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title, col, tag, valor, due, intType }),
    });
    if (res.ok) {
      const data = await res.json();
      serverId = data.id;
    }
  } catch (e) { console.error('Erro ao salvar card na API:', e); }

  addCardToState({ id: serverId, title, col, tag, valor, due, intType });
